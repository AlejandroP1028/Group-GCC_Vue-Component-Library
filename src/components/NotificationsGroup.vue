<template>
  <Transition name="fade">
    <div
      v-if="visible"
      :id="`toast-${type}`"
      class="flex items-center w-full max-w-xs p-4 mb-4 rounded-full shadow"
      :class="[computedClass, notificationBgClass, positioningClass, notificationTextColorClass]"
      role="alert"
    >
      <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-full" :class="iconClass">
...
export default {
  ...
  props: {
    ...
    position: {
      ...
      validator: (value) => [
        'top-left', 'top-right', 'bottom-right', 'bottom-left', 'center', 'pTop-left', 'pTop-right', 'pBottom-right', 'pBottom-left', 'pCenter'].includes(value),
    },
  },
  data() {
    return {
