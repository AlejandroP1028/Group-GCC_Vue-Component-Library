<template>
  <div class="relative w-full carousel-container" :class="{'border-blue-500 dark:border-gray-700': !isDarkMode }" data-carousel="static">
  
        <!-- Carousel wrapper -->
        <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
          <div v-for="(item, index) in items" :key="index" :class="itemClasses(index)" data-carousel-item>
            <img :src="item.src" :alt="item.alt" class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
          </div>
        </div>
        <!-- Slider indicators -->
