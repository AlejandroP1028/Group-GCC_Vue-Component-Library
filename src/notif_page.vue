      ...
      <div class="relative w-auto mb-16">
          ...
          <template #content>
            <div class="flex flex-col items-center mt-4 h-full">
              <div class="flex space-x-4">
                <Button color="sky" :class="'w-9/10'" @click="showNotification">Show Notification</Button>
              </div>
        ...
        <Section header="Notification Positions" @sectioncreated="addLink" body="The GCC notification component features a default notification style that seamlessly fades out for automatic dismissal.">
                <template #content>
                    <div class="flex flex-col items-center mt-4 h-full">
                        <div class="flex space-x-4">
                            <Button color="sky" :class="'w-9/10'" @click="showPositions('absolute')">Show Absolute Positions</Button>
                            <Button color="sky" :class="'w-9/10'" @click="showPositions('parent')">Show Parent Positions</Button>
                        </div>
                        <div :class="{ 'dark': defaultDark }" class="relative bg-gray-200 dark:bg-gray-800 transition-all duration-300 ease-in-out mt-4 w-full h-96 p-4 overflow-hidden rounded-lg shadow-lg border border-blue-600/[.87]">
    ...
    methods: {
        ...
        showPositions(type) {
            const positions = type === 'absolute' 
                ? ['top-right', 'bottom-right', 'top-left', 'bottom-left', 'center'] 
                : ['pTop-right', 'pBottom-right', 'pTop-left', 'pBottom-left', 'pCenter'];

            for (let p of positions) {
                if (type === 'absolute') {
                    this.notificationPositionsA.push({ type: 'BlueBorder', message: 'This is a default notification.', position: p });
                } else {
                    this.notificationPositionsP.push({ type: 'SkyBorder', message: 'This is a default notification.', position: p });
                }
            }
        },
    },
